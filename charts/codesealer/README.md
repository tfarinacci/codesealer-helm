...
> NOTE: If you would like to install Codesealer in `Standard` mode (with a local Manager) issue the
>       following commands:
>
> ```bash
> helm install codesealer codesealer/codesealer --create-namespace --namespace codesealer-system \
...
> NOTE: If you would like to upgrade Codesealer in `Standard` mode (with a local Manager) issue the
